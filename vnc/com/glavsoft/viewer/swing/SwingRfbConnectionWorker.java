...
public class SwingRfbConnectionWorker extends SwingWorker<Void, String>
	...
	public Void doInBackground() throws Exception {
		...
		workingProtocol = new Protocol(reader, writer, new PasswordChooser(
				connectionString, parentWindow, this), rfbSettings);
	...
	public SwingRfbConnectionWorker(String predefinedPassword,
			ConnectionPresenter presenter, JFrame parentWindow,
	...
	protected void done() { // EDT
		try {
			...
			ClipboardControllerImpl clipboardController = new ClipboardControllerImpl(
					workingProtocol, rfbSettings.getRemoteCharsetName());
			clipboardController.setEnabled(rfbSettings
					.isAllowClipboardTransfer());
			rfbSettings.addListener(clipboardController);
			viewerWindow = viewerWindowFactory.createViewerWindow(
					workingProtocol, rfbSettings, uiSettings, connectionString,
					presenter);

			workingProtocol.startNormalHandling(this,
					viewerWindow.getSurface(), clipboardController);
	...
	private class PasswordChooser implements IPasswordRetriever {
		...
		private PasswordChooser(String connectionString, JFrame parentWindow,
				ConnectionWorker onCancel) {
			this.connectionString = connectionString;
