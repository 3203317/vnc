...
public class SshConnectionManager implements SshKnownHostsManager {
	...
	public int connect(ConnectionParams connectionParams)
			throws CancelConnectionException {
		...
		try {
			...
			Logger.getLogger(this.getClass().getName()).severe(
					"Cannot set JSCH known hosts: " + e.getMessage());
		}
		int port = 0;
		try {
			session = jsch.getSession(connectionParams.sshUserName,
					connectionParams.sshHostName,
					connectionParams.getSshPortNumber());
			UserInfo ui = new SwingSshUserInfo(parentWindow);
			session.setUserInfo(ui);
			session.connect();
			sync();
			session.setPortForwardingL(0, connectionParams.hostName,
			...
			SwingUtilities.invokeAndWait(new Runnable() {
				...
				public void run() {
					final JOptionPane pane = new JOptionPane(
							"Please enter the user name for SSH connection:",
							JOptionPane.QUESTION_MESSAGE,
							JOptionPane.OK_CANCEL_OPTION);
					pane.setWantsInput(true);
					final JDialog dialog = pane.createDialog(parentWindow,
							"SSH User Name");
					Utils.decorateDialog(dialog);
					dialog.setVisible(true);
					result.stringRes = pane.getInputValue() != null ? (String) pane
							.getInputValue() : "";
					result.intRes = pane.getValue() != null ? (Integer) pane
							.getValue() : JOptionPane.OK_CANCEL_OPTION;
					dialog.dispose();
				}
			});
	...
	private InputStream getKnownHostsStream() {
		...
		return new ByteArrayInputStream(sshNode.getByteArray(KNOWN_HOSTS,
				new byte[0]));
	}
	...
	public void sync() {
		...
		try {
			...
			Logger.getLogger(this.getClass().getName()).severe(
					"Cannot sync JSCH known hosts: " + e.getMessage());
		}
		Preferences sshNode = Preferences.userRoot().node(SSH_NODE);
		sshNode.putByteArray(KNOWN_HOSTS, out.toByteArray());
	}

	private void write(ByteArrayOutputStream out, String str)
			throws IOException {
		try {
