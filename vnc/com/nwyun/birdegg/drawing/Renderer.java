...
public abstract class Renderer {
	 ...
	 */
	public int drawCompactBytes(byte[] bytes, int offset, int x, int y,
	 ...
	 */
	public void drawColoredBitmap(int[] colors, int x, int y, int width,
			int height) {
		synchronized (lock) {
			int i = 0;
			for (int ly = y; ly < y + height; ++ly) {
	...
	public void drawUncaliberedRGBLine(byte[] bytes, int x, int y, int width) {
		synchronized (lock) {
			...
			for (int i = 3, pixelsOffset = y * this.width + x; pixelsOffset < end; ++pixelsOffset) {
				...
				// (0xff & bytes[i++]) << 8 |
				// 0xff & bytes[i++];
				(0xff & 255 * (colorDecoder.redMax & bytes[i++])
						/ colorDecoder.redMax) << 16
						| (0xff & 255 * (colorDecoder.greenMax & bytes[i++])
								/ colorDecoder.greenMax) << 8
						| 0xff
						& 255
						* (colorDecoder.blueMax & bytes[i++])
						/ colorDecoder.blueMax;
			}
		}
	}
	...
	public void drawBytesWithPalette(byte[] buffer,
			FramebufferUpdateRectangle rect, int[] palette, int paletteSize) {
		synchronized (lock) {
			// 2 colors
			if (2 == paletteSize) {
				int dx, dy, n;
	...
	public void copyRect(int srcX, int srcY, FramebufferUpdateRectangle dstRect) {
		...
		synchronized (lock) {
			for (int y = startSrcY; y != endSrcY; y += deltaY) {
				System.arraycopy(pixels, y * width + srcX, pixels, dstY * width
	 ...
	 *            colour to fill with
	 * @param rect
	 *            rectangle region positions and dimensions
	 */
	...
	/**
	 * Reads color bytes (PIXEL) from reader, returns int combined RGB value
	...
	public void createCursor(int[] cursorPixels, FramebufferUpdateRectangle rect)
		...
		synchronized (cursor.getLock()) {
			cursor.createCursor(cursorPixels, rect.x, rect.y, rect.width,
					rect.height);
		}
	}
