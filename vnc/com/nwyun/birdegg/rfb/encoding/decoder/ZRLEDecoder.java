...
public class ZRLEDecoder extends ZlibDecoder {
	...
	public void decode(Reader reader, Renderer renderer,
		...
		for (int tileY = rect.y; tileY < maxY; tileY += MAX_TILE_SIZE) {
			...
			for (int tileX = rect.x; tileX < maxX; tileX += MAX_TILE_SIZE) {
				...
				// 2 to 16 for raw packed palette data, 130 to 255 for Palette
				// RLE (subencoding - 128)
				int paletteSize = subencoding & 127;
				offset += readPalette(bytes, offset, renderer, paletteSize);
				if (1 == subencoding) { // A solid tile consisting of a single
										// colour
					renderer.fillRect(palette[0], tileX, tileY, tileWidth,
							tileHeight);
					continue;
				}
				if (isRle) {
					if (0 == paletteSize) { // subencoding == 128 (or
											// paletteSize == 0) - Plain RLE
						offset += decodePlainRle(bytes, offset, renderer,
								tileX, tileY, tileWidth, tileHeight);
					} else {
						offset += decodePaletteRle(bytes, offset, renderer,
								tileX, tileY, tileWidth, tileHeight);
					}
				} else {
					if (0 == paletteSize) { // subencoding == 0 (or paletteSize
											// == 0) - raw CPIXEL data
						offset += decodeRaw(bytes, offset, renderer, tileX,
								tileY, tileWidth, tileHeight);
					} else {
						offset += decodePacked(bytes, offset, renderer,
	...
	private int decodePlainRle(byte[] bytes, int offset, Renderer renderer,
		...
		while (decodedOffset < decodedEnd) {
			...
			renderer.fillColorBitmapWithColor(decodedBitmap, decodedOffset,
					rlength, color);
			decodedOffset += rlength;
		}
		renderer.drawColoredBitmap(decodedBitmap, tileX, tileY, tileWidth,
				tileHeight);
		return index - offset;
	}
	...
	private int decodePacked(byte[] bytes, int offset, Renderer renderer,
		...
		for (int i = 0; i < tileHeight; ++i) {
			...
			while (decodedOffset < decodedRowEnd) {
				...
				int index = byteProcessed >> bitsRemain
	...
	private int decodeRaw(byte[] bytes, int offset, Renderer renderer,
		...
		return renderer.drawCompactBytes(bytes, offset, tileX, tileY,
	...
	private int readPalette(byte[] bytes, int offset, Renderer renderer,
		...
		for (int i = 0; i < paletteSize; ++i) {
			palette[i] = renderer.getCompactPixelColor(bytes, offset + i
					* bytesPerCPixel);
		}
